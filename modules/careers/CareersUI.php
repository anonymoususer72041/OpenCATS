...
class CareersUI extends UserInterface
{
    ...
    private function careersPage()
    {
        ...
        else if ($p == 'registeredCandidateProfile' && $isRegistrationEnabled)
        {
            ...
            $selectedCountryCode = strtoupper(trim((string) $candidate['country']));
            if (strlen($selectedCountryCode) != 2 || !isset($GLOBALS['countries'][$selectedCountryCode]))
            {
                $selectedCountryCode = '';
            }

            $countrySelectHTML = '<select id="country" name="country" class="inputBoxNormal">';
            $countrySelectHTML .= '<option value=""></option>';
            foreach ($GLOBALS['countries'] as $countryCode => $countryName)
            {
                $countrySelectHTML .= '<option value="'
                    . htmlspecialchars((string) $countryCode, ENT_QUOTES | ENT_SUBSTITUTE, HTML_ENCODING)
                    . '"';
                if ($countryCode == $selectedCountryCode)
                {
                    $countrySelectHTML .= ' selected="selected"';
                }
                $countrySelectHTML .= '>'
                    . htmlspecialchars((string) $countryName, ENT_QUOTES | ENT_SUBSTITUTE, HTML_ENCODING)
                    . '</option>';
            }
        ...
        else if ($p == 'applyToJob' || isset($_POST[$id='applyToJobSubAction']) && $_POST[$id] != '')
        {
            ...
            $selectedCountryCode = strtoupper(trim((string) $country));
            if (strlen($selectedCountryCode) != 2 || !isset($GLOBALS['countries'][$selectedCountryCode]))
            {
                $selectedCountryCode = '';
            }
